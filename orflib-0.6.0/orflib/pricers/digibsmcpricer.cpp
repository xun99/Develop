/**
@file  digibsmcpricer.cpp
@brief Implementation of the DigiBsMcPricer class for digital options 
*/
...
DigiBsMcPricer::DigiBsMcPricer(SPtrProduct prod,
                               ...
                               double spot,
                               McParams mcparams)
: prod_(prod), discyc_(discountCurve), divyld_(divYield), volTermStruct_(volTermStruct),
spot_(spot), mcparams_(mcparams)
{
  ...
  for (size_t i = 0; i < fixtimes.size(); ++i) {
    ...
    double vol = volTermStruct_->spotVol(t2);
    double var = vol * vol * (t2 - t1);
    stdevs_[i] = sqrt(var);
...
double DigiBsMcPricer::processOnePath(Matrix& pricePath)
{
