...
LPXLFOPER EXCEL_EXPORT xlOrfWorstOfDigiBSMC(LPXLFOPER xlPayoffType,
                                            LPXLFOPER xlStrike,
                                            LPXLFOPER xlFixingTime,
                                            LPXLFOPER xlExpirationTime,
                                            LPXLFOPER xlSpots,
                                            LPXLFOPER xlDiscountCrv,
                                            LPXLFOPER xlDivYields,
                                            LPXLFOPER xlVolatilities,
                                            ...
                                            LPXLFOPER xlHeaders)
{
  ...
  int payoffType = XlfOper(xlPayoffType).AsInt();
  double strike = XlfOper(xlStrike).AsDouble();
  double fixingTime = XlfOper(xlFixingTime).AsDouble();
  double expirationTime = XlfOper(xlExpirationTime).AsDouble();
  Vector spots = xlOperToVector(XlfOper(xlSpots));
  ...
  // create the product
  size_t numAssets = spots.size();
  SPtrProduct spprod(new WorstOfDigitalCallPut(payoffType, strike, fixingTime, expirationTime, numAssets));
  // create the pricer
  MultiAssetBsMcPricer bsmcpricer(spprod, spyc, divYields, vols, spots, correlMat, mcparams);
  // create the statistics calculator
  MeanVarCalculator<double*> sc(bsmcpricer.nVariables());
