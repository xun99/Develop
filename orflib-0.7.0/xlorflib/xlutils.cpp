...
orf::McParams xlOperToMcParams(xlw::XlfOper xlRange)
{
  ...
  for (RW i = 0; i < nr; ++i) {
    ...
    // recognize the name “ControlVarType” and the values “NONE” or “ANTITHETIC” and set the mcparams field accordingly
    else if (paramname == "CONTROLVARTYPE") {
      std::string paramvalue = xlRange(i, 1).AsString();
      paramvalue = orf::trim(paramvalue);
      std::transform(paramvalue.begin(), paramvalue.end(), paramvalue.begin(), ::toupper);
      if (paramvalue == "NONE")
        mcparams.controlVarType = McParams::ControlVarType::NONE;
      else if (paramvalue == "ANTITHETIC")
